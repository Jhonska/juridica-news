...
async function uploadDatabase() {
  ...
  if (compressed.length > 50 * 1024 * 1024) {
    console.error('❌ Compressed file still exceeds 50MB limit!');
    process.exit(1);
  }

  console.log('\n📤 Step 2: Uploading to Railway...');

  try {
    const response = await axios.post(
      `${RAILWAY_URL}/api/migration/upload-database`,
      {
    ...
    console.error('❌ Database upload failed:', error.response?.data || error.message);
    throw error;
  }
}
